...
Paste this into the Overrides field:
...
> **Note on tile ID:** The override CSS uses `.qq` throughout, which matches the default tile ID `qq`. If you change the tile ID in Basic Settings, update the CSS class names to match.

### Basic Settings (for reference)

This is my full exported Basic Settings string. Before importing, you'll need to change at minimum:
- `#tt#:Birds at Riverside` → your tile title
- The station ID inside `#ft#` (the footer link) → your station ID

Also note that the `Import Style` button may silently fail — if nothing changes after importing, enter the key settings manually (colors, font size, borders, etc.).
